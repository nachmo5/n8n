...
export function attachRuntimeSkillEnvironment(workspace: Workspace, env: NodeJS.ProcessEnv): void {
	const sandbox = workspace.sandbox;
	if (!sandbox) return;

	const previous = sandbox.getDefaultCommandEnv?.bind(sandbox);
	sandbox.getDefaultCommandEnv = () => ({
		...(previous?.() ?? {}),
		...env,
	});
}
...
export async function materializeRuntimeSkillsIntoWorkspace({
	...
	await writeWorkspaceFile(workspace, registryPath, `${JSON.stringify(registry, null, 2)}\n`);

	const defaultSkill =
		materialized.find((skill) => skill.name === 'workflow-auditor') ?? materialized[0];
